...
<head>
  ...
  <style>
    ...
    .auto-assign-active .manual-assign-btn { display: none; }
    .auto-assign-badge { display: none; }
    .auto-assign-active .auto-assign-badge { display: inline-flex; }
  </style>
</head>
                ...
                <td class="px-5 py-4">
                  <div class="flex items-center justify-end gap-1.5">
                    ...
                    <button onclick='openAssignDrawer(<?php echo json_encode($b); ?>)'
                      ...
                      class="manual-assign-btn flex items-center gap-1.5 px-3 py-1.5 rounded-lg <?php echo $b['status'] === 'Unassigned' ? 'bg-black text-white hover:bg-gray-800' : 'border border-gray-200 text-gray-600 hover:bg-gray-100'; ?> text-xs font-medium transition-all">
                      <i data-lucide="user-plus" class="w-3.5 h-3.5"></i>
                      <?php echo $b['status'] === 'Unassigned' ? 'Assign' : 'Reassign'; ?>
                    </button>
                    
                    <?php if($b['status'] === 'Unassigned'): ?>
                    <span class="auto-assign-badge flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-amber-50 text-amber-600 text-xs font-medium border border-amber-100/50">
  ...
  <div class="px-6 pt-5 pb-3 flex-shrink-0">
    <div class="flex items-center justify-between mb-3">
      <div class="flex items-center gap-2">
        ...
        <button onclick="toggleSelectAll()" class="text-[10px] font-bold bg-black text-white px-2.5 py-1 rounded-lg hover:bg-black shadow-sm transition-all uppercase tracking-wider">Select All</button>
      </div>
...
<script>
  ...
  function toggleSelectAll() {
    const visibleCards = Array.from(document.querySelectorAll('.pro-card')).filter(c => c.style.display !== 'none');
    const allSelected = visibleCards.length > 0 && visibleCards.every(c => c.classList.contains('selected'));
    
    if (allSelected) {
      // Deselect all visible
      visibleCards.forEach(c => {
        const id = parseInt(c.dataset.proId);
        const idx = selectedPros.findIndex(p => p.id === id);
        if (idx > -1) {
          selectedPros.splice(idx, 1);
      ...
      visibleCards.forEach(c => {
        const id = parseInt(c.dataset.proId);
        const nameNode = c.querySelector('p.text-sm.font-semibold');
        const name = nameNode ? nameNode.textContent : 'Professional';
  ...
  function bulkAutoAssignSweep() {
    const unassignedCount = visibleRows.filter(r => r.dataset.status === 'Unassigned').length;
    if (unassignedCount === 0) return;

    // Simulate batch processing
    setTimeout(() => {
      Swal.fire({
        title: 'Auto-Assignment Sweep',
        text: `Processing ${unassignedCount} unassigned bookings...`,
        timer: 1500,
        showConfirmButton: false,
  ...
  function autoAssignProfessional(booking) {
    // Basic auto-assign logic: find first available pro in the same city with highest rating
    const pros = JSON.parse('<?php echo json_encode($professionals); ?>');
    const availablePros = pros.filter(p => p.available && p.city === booking.city);
    
    // Sort by rating desc
    availablePros.sort((a, b) => b.rating - a.rating);

    if (availablePros.length === 0) {
      Swal.fire({
        title: 'Auto-Assign Failed',
        text: `No available professionals found in ${booking.city}. Switching to manual mode.`,
        icon: 'warning',
        confirmButtonColor: '#000'
      });
      // Fallback to manual - uncheck toggle for this action if you want, or just open drawer
      document.getElementById('auto-assign-toggle').checked = false;
      openAssignDrawer(booking);
      document.getElementById('auto-assign-toggle').checked = true; // restore toggle after opening
      return;
    }

    const assignedPro = availablePros[0];

    Swal.fire({
      title: 'Assigning...',
      html: `Automatically assigning <strong>${assignedPro.name}</strong> to booking <strong>#${booking.id}</strong>...`,
    ...
    setTimeout(() => {
      Swal.fire({
        title: 'Auto-Assigned!',
        html: `<strong>${assignedPro.name}</strong> has been successfully assigned to booking <strong>#${booking.id}</strong>.`,
        icon: 'success',
        confirmButtonColor: '#000'
      });
    }, 1200);
  ...
  (function init() { 
    ...
    document.getElementById('auto-assign-toggle').checked = autoEnabled;

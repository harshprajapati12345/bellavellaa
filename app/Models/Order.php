...
class Order extends Model
{
    ...
    protected $fillable = [
        'order_number', 'customer_id', 'professional_id',
        'address', 'city', 'zip', 'latitude', 'longitude',
        'scheduled_date', 'scheduled_slot',
        'subtotal_paise', 'discount_paise', 'tax_paise', 'total_paise', 'coins_used',
    ...
    public function getSubtotalAttribute(): float { return $this->subtotal_paise / 100; }
    public function getDiscountAttribute(): float { return $this->discount_paise / 100; }
    public function getTaxAttribute(): float { return $this->tax_paise / 100; }
    public function getTotalAttribute(): float { return $this->total_paise / 100; }
    ...
    public function assignments(): HasMany { return $this->hasMany(OrderAssignment::class); }
    public function otps(): HasMany { return $this->hasMany(OrderOtp::class); }
    public function payments(): HasMany { return $this->hasMany(Payment::class); }
    public function refunds(): HasMany { return $this->hasMany(Refund::class); }

    public function scopePending($q) { return $q->where('status', 'pending'); }
    public function scopeActive($q) { return $q->whereNotIn('status', ['completed', 'cancelled']); }

...
// TODO: $category = db_row('SELECT * FROM categories WHERE id = ?', [$id]);
$category = ['id'=>$id,'name'=>'Bridal','description'=>'All bridal beauty services.','parent_id'=>1,'status'=>1,'image'=>''];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $category['parent_id']   = intval($_POST['parent_id'] ?? 0);
    $category['status']      = isset($_POST['status']) ? 1 : 0;
    if (!$category['name']) $errors[] = 'Category name is required.';
    if (empty($errors)) {
        // TODO: UPDATE categories SET name=?, description=?, parent_id=?, status=? WHERE id=?
        $success = true;
    }
}
?>
  ...
  <main class="flex-1 lg:ml-72 p-4 lg:p-8 overflow-y-auto">
    ...
    <div class="flex flex-col gap-6">
      <div class="flex items-center gap-4">
        <a href="/bellavella/categories/" class="w-9 h-9 rounded-xl bg-white border border-gray-200 flex items-center justify-center hover:bg-gray-50 transition-all shadow-sm"><i data-lucide="arrow-left" class="w-4 h-4 text-gray-600"></i></a>
        <div class="flex-1">
          <h2 class="text-2xl font-semibold text-gray-900 tracking-tight">Edit Category</h2>
          <p class="text-sm text-gray-400 mt-0.5"><?php echo htmlspecialchars($category['name']); ?></p>
        </div>
      </div>
      <?php if ($success): ?><div class="flex items-center gap-3 bg-emerald-50 border border-emerald-200 text-emerald-700 px-5 py-3.5 rounded-2xl text-sm font-medium"><i data-lucide="check-circle" class="w-4 h-4"></i> Category updated!</div><?php endif; ?>
      ...
      <form method="POST" enctype="multipart/form-data" class="bg-white rounded-[2rem] shadow-[0_2px_16px_rgba(0,0,0,0.04)] overflow-hidden">
        <div class="p-8 border-b border-gray-100">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div class="space-y-6">
                    ...
                    <textarea name="description" rows="5" class="form-input resize-none"><?php echo htmlspecialchars($category['description']); ?></textarea>
                    </div>
                </div>
                ...
                <div class="space-y-6">
                    ...
                    <select name="parent_id" class="form-input">
                        <option value="0" <?php echo ($category['parent_id'] == 0) ? 'selected' : ''; ?>>— None (Top-level parent) —</option>
                    ...
                    <p class="text-xs text-gray-400 mt-1.5 ml-1">Select Luxe or Prime to nest under a parent, or leave blank for a top-level category.</p>
                    </div>
                    <div>
                    ...
                    <div class="flex gap-4 items-start">
                        ...
                        <?php if ($category['image']): ?>
                            <img src="<?php echo htmlspecialchars($category['image']); ?>" class="w-32 h-32 object-cover rounded-2xl border border-gray-100" alt="">
                        <?php endif; ?>
        ...
        <div class="flex items-center justify-between px-8 py-5 bg-gray-50/50">
          <a href="/bellavella/categories/delete.php?id=<?php echo $id; ?>" onclick="return confirm('Delete this category?')" class="btn btn-danger text-sm"><i data-lucide="trash-2" class="w-4 h-4"></i> Delete</a>

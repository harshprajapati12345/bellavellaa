  ...
  <div class="flex flex-col gap-6">
    ...
    <div class="flex items-center gap-4">
      ...
      <div>
        ...
        <p class="text-sm text-gray-400 mt-0.5">Edit: {{ $package->name }}</p>
      </div>
    </div>
    ...
    <form method="POST" action="{{ route('packages.update', $package->id) }}" enctype="multipart/form-data"
      ...
      <div class="bg-white rounded-[2rem] border border-gray-100 shadow-sm overflow-hidden mb-6 section-card">
        ...
        <div class="px-8 pb-8">
          <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div class="space-y-5">
              <div>
                <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2 ml-1">Package Name
              ...
              <div class="flex items-center gap-6 mt-4">
                <label class="flex items-center gap-2 cursor-pointer group">
                  <div class="relative flex items-center">
                    <input type="checkbox" name="featured" id="featured" {{ old('featured', $package->featured) ? 'checked' : '' }}
                      class="peer h-5 w-5 cursor-pointer appearance-none rounded border border-gray-300 bg-white checked:border-black checked:bg-black transition-all">
                    <i data-lucide="check"
                      class="absolute left-1 w-3 h-3 text-white transition-opacity opacity-0 peer-checked:opacity-100 pointer-events-none"></i>
                    <input type="hidden" name="featured_hidden" value="0">
                  </div>
                  <span class="text-sm font-medium text-gray-700 group-hover:text-black">Featured Package</span>
                </label>
              </div>
            </div>
            <!-- Right: Image Upload -->
            <div>
              <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2 ml-1">Package
                Image</label>
              <div id="dropZone1"
                class="drop-zone relative flex flex-col items-center justify-center w-full h-44 border-2 border-dashed border-gray-200 rounded-2xl cursor-pointer hover:border-gray-400 hover:bg-gray-50/60 transition-all group {{ $package->image ? 'hidden' : '' }}"
      ...
      <div class="bg-white rounded-[2rem] border border-gray-100 shadow-sm overflow-hidden mb-6 section-card">
        ...
        <div class="px-8 pb-8 space-y-5">
          ...
          <!-- Service Search + Add -->
          <div>
            <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2 ml-1">Select
              Service</label>
            <div class="flex items-center gap-3">
              <div class="relative flex-1">
                ...
                <div id="serviceDropdown"
                  class="hidden absolute top-full left-0 right-0 z-40 max-h-60 overflow-y-auto bg-white border border-gray-200 rounded-xl shadow-lg mt-1">
                  @foreach($services as $svc)
                    <div
                      class="svc-dropdown-item px-4 py-3 cursor-pointer flex items-center justify-between hover:bg-gray-50 transition-colors"
                      data-id="{{ $svc->id }}" data-name="{{ $svc->name }}" data-duration="{{ $svc->duration }}"
                      data-price="{{ $svc->price }}" onclick="selectService(this)">
                      <div>
                  ...
                  @endforeach
                  <div id="noResultsItem" class="hidden px-4 py-3 text-sm text-gray-400 text-center">No services found
                  </div>
                </div>
              </div>
          ...
          <div>
            <div id="selectedServicesHeader" class="hidden flex items-center gap-3 mb-3 px-1">
              ...
              <span
                class="w-24 text-[10px] font-semibold text-gray-400 uppercase tracking-widest text-center">Duration</span>
      ...
      <div class="bg-white rounded-[2rem] border border-gray-100 shadow-sm overflow-hidden mb-6 section-card">
        ...
        <div class="px-8 pb-8 space-y-5">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div>
              <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2 ml-1">Total Original
                Price</label>
              <div class="relative">
                <span class="absolute left-4 top-1/2 -translate-y-1/2 text-sm text-gray-400 font-medium">₹</span>
                <input type="text" id="totalOriginalPrice" value="0"
                  class="w-full px-8 py-3 rounded-xl border border-gray-100 bg-gray-50 text-gray-500 cursor-not-allowed text-sm font-medium"
                  readonly>
              </div>
              <p class="text-[11px] text-gray-400 mt-1.5 ml-1">Sum of all included services</p>
            </div>
            <div>
              <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2 ml-1">Total
                Duration</label>
              <div class="relative">
                <span class="absolute left-4 top-1/2 -translate-y-1/2 text-sm text-gray-400">
                  <i data-lucide="clock" class="w-4 h-4"></i>
                </span>
                <input type="text" id="totalDurationDisplay" value="0 min"
                  class="w-full px-10 py-3 rounded-xl border border-gray-100 bg-gray-50 text-gray-500 cursor-not-allowed text-sm font-medium"
                  readonly>
                <input type="hidden" name="duration" id="totalDurationHidden"
          ...
          <div>
            <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2 ml-1">Package Price <span
                class="text-red-400">*</span></label>
            <div class="relative">
              <span class="absolute left-4 top-1/2 -translate-y-1/2 text-sm text-gray-500 font-semibold">₹</span>
              <input type="number" name="package_price" id="packagePriceInput"
                value="{{ old('package_price', $package->price) }}" placeholder="0.00" min="0" step="0.01"
          ...
          <div>
            <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2 ml-1">Fixed Discount
              (%)</label>
            <input type="number" name="discount" id="discountInput" value="{{ old('discount', $package->discount) }}"
              min="0" max="100"
              class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-black/5 focus:border-black/50 text-sm bg-white transition-all"
              oninput="updateSavings()">
            <p class="text-[11px] text-gray-400 mt-1.5 ml-1">Optional override for discount percentage</p>
          </div>

          <!-- Savings display -->
          <div id="savingsDisplay" class="hidden">
            <div
              class="rounded-2xl border border-emerald-100 bg-emerald-50/50 px-5 py-4 flex items-center justify-between">
              <div class="flex items-center gap-3">
      ...
      <div class="bg-white rounded-[2rem] border border-gray-100 shadow-sm overflow-hidden mb-6 section-card">
        <div class="px-8 pt-7 pb-2">
          <div class="flex items-center gap-3 mb-6">
            <div class="w-8 h-8 rounded-lg bg-gray-900 text-white flex items-center justify-center text-sm font-bold">4
            </div>
        ...
        <div class="px-8 pb-8 space-y-5">
          <div>
            <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2 ml-1">Description
              Title</label>
            <input type="text" name="desc_title" value="{{ old('desc_title', $package->desc_title) }}"
              placeholder="e.g. Complete Bridal Glow Experience"
              class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-black/5 focus:border-black/50 text-sm bg-white transition-all">
          </div>
          <div>
            <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2 ml-1">Description
            ...
            <div class="border border-gray-200 rounded-2xl overflow-hidden focus-within:border-black/50 transition-all">
              <div class="flex items-center gap-1 px-3 py-2 bg-gray-50/80 border-b border-gray-200">
                <button type="button" onclick="execCmd('bold', 'descEditor')"
                  class="w-8 h-8 rounded-lg hover:bg-gray-200 flex items-center justify-center transition-colors"
                  title="Bold"><b>B</b></button>
                <button type="button" onclick="execCmd('italic', 'descEditor')"
                  class="w-8 h-8 rounded-lg hover:bg-gray-200 flex items-center justify-center transition-colors"
                  title="Italic"><i>I</i></button>
                <button type="button" onclick="execCmd('underline', 'descEditor')"
                  class="w-8 h-8 rounded-lg hover:bg-gray-200 flex items-center justify-center transition-colors"
                  title="Underline"><u>U</u></button>
                <div class="w-px h-5 bg-gray-200 mx-1"></div>
                <button type="button" onclick="execCmd('insertUnorderedList', 'descEditor')"
                  class="w-8 h-8 rounded-lg hover:bg-gray-200 flex items-center justify-center transition-colors"
                  title="Bullet List"><i data-lucide="list" class="w-4 h-4"></i></button>
              </div>
              <div id="descEditor" contenteditable="true"
                class="px-5 py-4 min-h-[160px] text-sm text-gray-800 outline-none"
                data-placeholder="Describe this package and what's included…">
                {!! old('desc_content', $package->description) !!}
              </div>
              <input type="hidden" name="desc_content" id="descContentHidden">
            </div>
          </div>
          <div>
            <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2 ml-1">Description Image
              <span class="text-gray-400 font-normal normal-case">(Optional)</span></label>
            <div id="dropZoneDesc"
              class="drop-zone flex flex-col items-center justify-center w-full h-40 border-2 border-dashed border-gray-200 rounded-2xl cursor-pointer hover:border-gray-400 hover:bg-gray-50/60 transition-all group {{ $package->desc_image ? 'hidden' : '' }}"
              ...
              <div id="uploadPlaceholderDesc" class="flex flex-col items-center">
                <div
                  class="w-10 h-10 rounded-xl bg-gray-100 flex items-center justify-center mb-2 group-hover:bg-gray-200 transition-colors">
                  <i data-lucide="image-plus" class="w-5 h-5 text-gray-400"></i>
                </div>
                <p class="text-sm text-gray-500">Upload description image</p>
              </div>
              <input type="file" name="desc_image" id="descImageInput" accept="image/*" class="hidden"
                onchange="previewImg(this, 'descImgPreview', 'uploadPlaceholderDesc', 'dropZoneDesc')">
            </div>
            <div class="relative mt-3 {{ $package->desc_image ? '' : 'hidden' }}" id="descImgPreviewWrap">
              <img id="descImgPreview" class="w-full h-40 object-cover rounded-2xl border border-gray-100 shadow-sm"
                src="{{ $package->desc_image }}" alt="">
              <button type="button"
                class="absolute top-2 right-2 w-8 h-8 rounded-full bg-white/90 border border-gray-200 flex items-center justify-center text-gray-500 hover:bg-red-50 hover:text-red-500 hover:border-red-200 transition-all shadow-sm"
                onclick="removeImage('descImageInput','descImgPreview','descImgPreviewWrap','uploadPlaceholderDesc','dropZoneDesc')">
                <i data-lucide="x" class="w-4 h-4"></i>
              </button>
            </div>
          </div>
      ...
      <div class="bg-white rounded-[2rem] border border-gray-100 shadow-sm overflow-hidden mb-6 section-card">
        <div class="px-8 pt-7 pb-2">
          <div class="flex items-center gap-3 mb-6">
            <div class="w-8 h-8 rounded-lg bg-gray-900 text-white flex items-center justify-center text-sm font-bold">5
            </div>
        ...
        <div class="px-8 pb-8 space-y-5">
          <div>
            <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2 ml-1">Aftercare
              Content</label>
            <div class="border border-gray-200 rounded-2xl overflow-hidden focus-within:border-black/50 transition-all">
              <div class="flex items-center gap-1 px-3 py-2 bg-gray-50/80 border-b border-gray-200">
                <button type="button" onclick="execCmd('bold', 'aftercareEditor')"
                  class="w-8 h-8 rounded-lg hover:bg-gray-200 flex items-center justify-center transition-colors"
                  title="Bold"><b>B</b></button>
                <button type="button" onclick="execCmd('italic', 'aftercareEditor')"
                  class="w-8 h-8 rounded-lg hover:bg-gray-200 flex items-center justify-center transition-colors"
                  title="Italic"><i>I</i></button>
                <button type="button" onclick="execCmd('insertUnorderedList', 'aftercareEditor')"
                  class="w-8 h-8 rounded-lg hover:bg-gray-200 flex items-center justify-center transition-colors"
                  title="List"><i data-lucide="list" class="w-4 h-4"></i></button>
              </div>
              <div id="aftercareEditor" contenteditable="true"
                class="px-5 py-4 min-h-[120px] text-sm text-gray-800 outline-none"
                data-placeholder="Post-service care instructions…">
                {!! old('aftercare_content', $package->aftercare_content) !!}
              </div>
              <input type="hidden" name="aftercare_content" id="aftercareContentHidden">
            </div>
          </div>
          <div>
            <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2 ml-1">Aftercare Image
              <span class="text-gray-400 font-normal normal-case">(Optional)</span></label>
            <div id="dropZoneAfter"
              class="drop-zone flex flex-col items-center justify-center w-full h-36 border-2 border-dashed border-gray-200 rounded-2xl cursor-pointer hover:border-gray-400 hover:bg-gray-50/60 transition-all group {{ $package->aftercare_image ? 'hidden' : '' }}"
              ...
              <input type="file" name="aftercare_image" id="afterImageInput" accept="image/*" class="hidden"
                onchange="previewImg(this, 'afterImgPreview', 'uploadPlaceholderAfter', 'dropZoneAfter')">
            </div>
            <div class="relative mt-3 {{ $package->aftercare_image ? '' : 'hidden' }}" id="afterImgPreviewWrap">
              <img id="afterImgPreview" class="w-full h-36 object-cover rounded-2xl border border-gray-100 shadow-sm"
                src="{{ $package->aftercare_image }}" alt="">
              <button type="button"
                class="absolute top-2 right-2 w-8 h-8 rounded-full bg-white/90 border border-gray-200 flex items-center justify-center text-gray-500 hover:bg-red-50 hover:text-red-500 hover:border-red-200 transition-all shadow-sm"
                onclick="removeImage('afterImageInput','afterImgPreview','afterImgPreviewWrap','uploadPlaceholderAfter','dropZoneAfter')">
                <i data-lucide="x" class="w-4 h-4"></i>
              </button>
            </div>
          </div>
      ...
      <!-- STICKY ACTION BAR -->
      <div
        class="sticky bottom-8 z-30 bg-white/80 backdrop-blur-xl rounded-2xl border border-gray-100 shadow-xl px-8 py-4 flex items-center justify-between mt-8">
        <button type="button" onclick="deletePackage({{ $package->id }}, '{{ addslashes($package->name) }}')"
          class="px-5 py-2.5 rounded-xl border border-red-100 text-red-500 text-sm font-medium hover:bg-red-50 transition-all flex items-center gap-2">
          <i data-lucide="trash-2" class="w-4 h-4"></i> Delete Package
        </button>
        ...
        <div class="flex items-center gap-3">
          ...
          <button type="submit" name="form_action" value="draft"
            class="px-6 py-2.5 rounded-xl border border-gray-200 text-gray-700 text-sm font-medium hover:bg-gray-50 transition-all flex items-center gap-2">
            <i data-lucide="file-text" class="w-4 h-4"></i> Keep as Draft
          </button>
  ...
  <script>
    ...
    function filterServiceDropdown() {
      ...
      items.forEach(item => {
        ...
        const isSelected = serviceObjects.some(s => s.id === id);
    ...
    function addSelectedService() {
      ...
      serviceObjects.push({ ...pendingService });
      pendingService = null;
    ...
    function removeSelectedService(id) {
      serviceObjects = serviceObjects.filter(s => s.id !== id);
      renderSelectedServices();
      recalcTotals();
    }
    ...
    function renderSelectedServices() {
      ...
      serviceObjects.forEach((svc, idx) => {
    ...
    function recalcTotals() {
      ...
      serviceObjects.forEach(s => {
        totalP += Number(s.price);
        totalD += Number(s.duration);
      });

      document.getElementById('totalOriginalPrice').value = totalP.toLocaleString('en-IN', { minimumFractionDigits: 2 });
    ...
    function updateSavings() {
      ...
      if (pkgPrice > 0 && original > 0) {
        ...
        // Auto-update discount field if not manually overridden
        // document.getElementById('discountInput').value = pct; 

        savingsEl.classList.toggle('hidden', saved <= 0);
      } else {

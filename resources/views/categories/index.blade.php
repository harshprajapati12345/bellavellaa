    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
      <div
        ...
        <div>
          ...
          <p class="text-3xl font-bold text-gray-900">{{ $totalCats }}</p>
        ...
        <div class="w-11 h-11 rounded-2xl bg-gray-100 flex items-center justify-center flex-shrink-0"><i
            data-lucide="folder" class="w-5 h-5 text-gray-600"></i></div>
      </div>
      <div
        class="stat-card bg-white rounded-2xl p-5 shadow-[0_2px_16px_rgba(0,0,0,0.04)] flex items-center justify-between gap-4">
        <div>
          <p class="text-xs font-semibold text-emerald-500 uppercase tracking-widest mb-1">Active</p>
          <p class="text-3xl font-bold text-gray-900">{{ $totalActive }}</p>
          <p class="text-xs text-gray-400 mt-0.5">Live categories</p>
        </div>
      ...
      <div
        class="stat-card bg-white rounded-2xl p-5 shadow-[0_2px_16px_rgba(0,0,0,0.04)] flex items-center justify-between gap-4">
        <div>
          <p class="text-xs font-semibold text-gray-400 uppercase tracking-widest mb-1">Services</p>
          <p class="text-3xl font-bold text-gray-900">{{ $totalSvcs }}</p>
          <p class="text-xs text-gray-400 mt-0.5">Active services</p>
        </div>
        <div class="w-11 h-11 rounded-2xl bg-gray-100 flex items-center justify-center flex-shrink-0"><i
            data-lucide="store" class="w-5 h-5 text-gray-600"></i></div>
      </div>
      <div
        class="stat-card bg-white rounded-2xl p-5 shadow-[0_2px_16px_rgba(0,0,0,0.04)] flex items-center justify-between gap-4">
        <div>
          <p class="text-xs font-semibold text-violet-500 uppercase tracking-widest mb-1">Top Performing</p>
          <p class="text-lg font-bold text-gray-900 mt-1">{{ $topCategory->name ?? 'N/A' }}</p>
          <p class="text-xs text-gray-400 mt-0.5">{{ $topCategory ? number_format($topCategory->bookings_count) : 0 }}
            bookings</p>
        </div>
          ...
          <table class="w-full min-w-[640px]">
            <thead>
              <tr class="border-b border-gray-100 bg-gray-50/80">
                <th class="px-5 py-4 text-left text-xs font-semibold text-gray-400 uppercase tracking-widest">Category
                </th>
                <th class="px-5 py-4 text-left text-xs font-semibold text-gray-400 uppercase tracking-widest">Services
                </th>
                <th class="px-5 py-4 text-left text-xs font-semibold text-gray-400 uppercase tracking-widest">Bookings
                </th>
                <th class="px-5 py-4 text-left text-xs font-semibold text-gray-400 uppercase tracking-widest">Status</th>
                <th class="px-5 py-4 text-right text-xs font-semibold text-gray-400 uppercase tracking-widest">Actions
                </th>
              </tr>
            </thead>
            <tbody id="categories-tbody">
              @foreach($categories as $cat)
                <tr class="table-row border-b border-gray-50" data-id="{{ $cat->id }}"
                  data-name="{{ strtolower($cat->name) }}" data-title="{{ $cat->name }}"
                  data-status="{{ $cat->status ?? 'Active' }}" data-slug="{{ $cat->slug ?? Str::slug($cat->name) }}"
                  data-image="{{ $cat->image }}" data-services="{{ $cat->services_count ?? 0 }}"
                  data-bookings="{{ $cat->bookings_count ?? 0 }}" data-description="{{ $cat->description }}"
                  data-created="{{ \Carbon\Carbon::parse($cat->created_at)->format('d M Y') }}">
                  <td class="px-5 py-4">
                    <div class="flex items-center gap-3">
                      <div class="w-10 h-10 rounded-xl overflow-hidden flex-shrink-0">
                        <img
                          src="{{ $cat->image ? (str_starts_with($cat->image, 'http') ? $cat->image : asset('storage/' . $cat->image)) : 'https://images.unsplash.com/photo-1596704017254-9b1b1b9e07f9?auto=format&fit=crop&w=400&q=80' }}"
                  ...
                  <td class="px-5 py-4">
                    <label class="toggle-switch">
                      <input type="checkbox" {{ ($cat->status ?? 'Active') === 'Active' ? 'checked' : '' }}

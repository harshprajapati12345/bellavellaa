...
@section('content')
  <div class="flex flex-col gap-6">
    <div class="flex items-center gap-4">
      ...
      <div class="flex-1">
        ...
        <p class="text-sm text-gray-400 mt-0.5">Editing: <span class="text-black font-medium">{{ $category->name }}</span>
        </p>
      </div>
    </div>
    ...
    <form method="POST" action="{{ route('categories.update', $category->id) }}" enctype="multipart/form-data"
          ...
          <div class="space-y-8">
            ...
            <div>
              ...
              <div class="flex gap-5 items-start">
                <label
                  class="flex flex-col items-center justify-center w-full h-40 border-2 border-dashed border-gray-100 rounded-[2.2rem] cursor-pointer hover:border-black/20 hover:bg-gray-50 transition-all flex-1 pb-2">
                ...
                <div class="w-40 h-40 relative group overflow-hidden rounded-[2.2rem]">
                  <img id="img-preview"
                    src="{{ $category->image ? (str_starts_with($category->image, 'http') ? $category->image : asset('storage/' . $category->image)) : 'https://images.unsplash.com/photo-1596704017254-9b1b1b9e07f9?auto=format&fit=crop&w=800&q=80' }}"
                    class="w-full h-full object-cover border border-gray-100" alt="">
                  <div
                    class="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center cursor-default">
      ...
      <div class="flex items-center justify-between px-10 py-6 bg-[#F9F9F9]/50">
        <button type="button" onclick="handleDelete()" class="btn btn-danger text-sm px-6">
  ...
  <form id="delete-cat-form" method="POST" action="{{ route('categories.destroy', $category->id) }}" class="hidden">
    @csrf @method('DELETE')
  </form>
@endsection

  ...
  <div class="flex flex-col gap-6">
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
      <div
        ...
        <div>
          <p class="text-xs font-semibold text-gray-400 uppercase tracking-widest mb-1">Professionals</p>
          <p class="text-3xl font-bold text-gray-900">{{ $totalPros }}</p>
        </div>
        <div class="w-11 h-11 rounded-2xl bg-gray-100 flex items-center justify-center flex-shrink-0"><i
            data-lucide="users" class="w-5 h-5 text-gray-600"></i></div>
      </div>
      <div
        class="stat-card bg-white rounded-2xl p-5 shadow-[0_2px_16px_rgba(0,0,0,0.04)] flex items-center justify-between gap-4">
        <div>
          <p class="text-xs font-semibold text-emerald-500 uppercase tracking-widest mb-1">Total Earnings</p>
          <p class="text-3xl font-bold text-gray-900">₹{{ number_format($totalEarnings / 1000, 0) }}K</p>
        </div>
        <div class="w-11 h-11 rounded-2xl bg-emerald-50 flex items-center justify-center flex-shrink-0"><i
            data-lucide="trending-up" class="w-5 h-5 text-emerald-500"></i></div>
      </div>
      <div
        class="stat-card bg-white rounded-2xl p-5 shadow-[0_2px_16px_rgba(0,0,0,0.04)] flex items-center justify-between gap-4">
        <div>
          <p class="text-xs font-semibold text-violet-500 uppercase tracking-widest mb-1">Commission</p>
          <p class="text-3xl font-bold text-gray-900">₹{{ number_format($totalCommission / 1000, 0) }}K</p>
        </div>
      ...
      <div
        class="stat-card bg-white rounded-2xl p-5 shadow-[0_2px_16px_rgba(0,0,0,0.04)] flex items-center justify-between gap-4">
        <div>
          <p class="text-xs font-semibold text-amber-500 uppercase tracking-widest mb-1">Avg Rating</p>
          <p class="text-3xl font-bold text-gray-900">{{ $avgRating }} ★</p>
        </div>
        <div class="w-11 h-11 rounded-2xl bg-amber-50 flex items-center justify-center flex-shrink-0"><i
            data-lucide="star" class="w-5 h-5 text-amber-500"></i></div>
      </div>
    </div>
        ...
        <table class="w-full min-w-[900px]">
          <thead>
            <tr class="border-b border-gray-100 bg-gray-50/80">
              ...
              <th class="px-5 py-4 text-left text-xs font-semibold text-gray-400 uppercase tracking-widest">Total Orders
              </th>
              ...
              <th class="px-5 py-4 text-left text-xs font-semibold text-gray-400 uppercase tracking-widest">Total Earnings
              </th>
          ...
          <tbody id="hist-tbody">
            @foreach($history as $h)
              <tr class="table-row border-b border-gray-50" data-id="{{ $h['id'] }}"
                data-name="{{ strtolower($h['name']) }}" data-avatar="{{ $h['avatar'] }}"
                data-category="{{ $h['category'] }}" data-total-orders="{{ $h['total_orders'] }}"
                data-completed="{{ $h['completed'] }}" data-cancelled="{{ $h['cancelled'] }}"
                data-total-earnings="{{ $h['total_earnings'] }}" data-total-commission="{{ $h['total_commission'] }}"
                data-payout-status="{{ $h['payout_status'] }}" data-rating="{{ $h['rating'] }}"
                data-monthly="{{ json_encode($h['monthly']) }}" data-reviews="{{ json_encode($h['reviews']) }}"
                onclick="openHistDrawer(this.dataset)">
                <td class="px-5 py-4">
                  <div class="flex items-center gap-3">
                    <img src="{{ $h['avatar'] }}"
                      class="w-10 h-10 rounded-full object-cover ring-2 ring-white ring-offset-1 ring-offset-gray-100 flex-shrink-0"
                      alt="">
                    <div>
                      <p class="text-sm font-semibold text-gray-900">{{ $h['name'] }}</p>
                      <span
                        class="text-xs font-medium px-2 py-0.5 rounded-full {{ $h['category'] === 'Luxe' ? 'bg-violet-50 text-violet-600' : 'bg-amber-50 text-amber-600' }}">{{ $h['category'] }}</span>
                    </div>
                  </div>
                </td>
                ...
                <td class="px-5 py-4">
                  <div class="flex items-center gap-2">
                    ...
                    <div class="progress-bar flex-1 min-w-[60px]">
                      <div class="progress-fill"
                        style="width:{{ $h['total_orders'] > 0 ? round($h['completed'] / $h['total_orders'] * 100) : 0 }}%">
                      </div>
                    </div>
                  </div>
                </td>
  ...
  <div id="drawer-panel"
    ...
    <div class="flex items-center justify-between px-6 pt-6 pb-5 border-b border-gray-100 flex-shrink-0">
      <div class="flex items-center gap-3">
        ...
        <div>
          <h3 id="hd-name" class="text-lg font-semibold text-gray-900"></h3>
          <p id="hd-category" class="text-sm text-gray-400"></p>
        </div>
      </div>
    ...
    <div class="flex-1 overflow-y-auto p-6 flex flex-col gap-6">
      ...
      <div class="bg-black rounded-2xl p-5 grid grid-cols-3 gap-4">
        <div class="text-center">
          <p id="hd-earnings" class="text-lg font-bold text-white"></p>
          <p class="text-xs text-gray-400 mt-0.5">Earnings</p>
        </div>
        <div class="text-center border-x border-white/10">
          <p id="hd-commission" class="text-lg font-bold text-violet-300"></p>
          <p class="text-xs text-gray-400 mt-0.5">Commission</p>
        </div>
        <div class="text-center">
          <p id="hd-payout" class="text-lg font-bold text-amber-300"></p>
          <p class="text-xs text-gray-400 mt-0.5">Payout Status</p>
        </div>
      </div>
    ...
    function openHistDrawer(h) {
      ...
      if (chart) {
        ...
        monthly.forEach(val => {
          ...
          const height = max > 0 ? Math.round((val / max) * 96) : 0;
          bar.style.height = Math.max(4, height) + 'px';
          bar.title = '₹' + (val || 0).toLocaleString('en-IN');

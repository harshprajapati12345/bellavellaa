...
<div class="flex flex-col gap-6">
  ...
  <form method="POST" action="{{ route('professionals.update', $professional->id) }}" enctype="multipart/form-data" class="bg-white rounded-[2rem] border border-gray-100 shadow-sm overflow-hidden">
    @csrf
    @method('PUT')
        ...
        <div class="md:col-span-2 space-y-6">
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
            ...
            <div>
              ...
              <input type="email" name="email" value="{{ old('email', $professional->email) }}" class="form-input" required>
            </div>
            <div>
            ...
            <div>
              ...
              <select name="category" class="form-input cursor-pointer">
                @foreach(['Makeup Artist','Hair Stylist','Nail Technician','Skincare Specialist','Wellness Expert'] as $cat)
                  <option value="{{ $cat }}" {{ old('category', $professional->category) === $cat ? 'selected' : '' }}>{{ $cat }}</option>
                @endforeach
              </select>
            </div>
            <div>
            ...
            <div>
              ...
              <input type="text" name="experience" value="{{ old('experience', $professional->experience) }}" class="form-input">
            </div>
            <div>
                <label class="form-label">Rating</label>
                <input type="number" name="rating" value="{{ old('rating', $professional->rating) }}" min="0" max="5" step="0.1" class="form-input">
            </div>
            <div class="sm:col-span-2">
              <label class="form-label">Bio</label>
              <textarea name="bio" rows="4" class="form-input resize-none">{{ old('bio', $professional->bio) }}</textarea>
            </div>
          </div>
        </div>
        ...
        <div class="space-y-6">
          ...
          <div>
            ...
            <div class="flex flex-col items-center gap-4">
              <div class="relative">
                <img id="img-preview" src="{{ $professional->avatar ?? 'https://ui-avatars.com/api/?name='.urlencode($professional->name).'&background=f3f4f6&color=6b7280&size=128' }}" 

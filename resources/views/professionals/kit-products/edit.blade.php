@extends('layouts.app')
@php $pageTitle = 'Edit Kit Product'; @endphp
...
@section('content')
  <div class="flex flex-col gap-6">
    <div class="flex items-center gap-4">
      ...
      <div>
        <h2 class="text-2xl font-semibold text-gray-900 tracking-tight">Edit Kit Product</h2>
        <p class="text-sm text-gray-400 mt-0.5">Modify details for {{ $kitProduct->name }}</p>
      </div>
    </div>
    ...
    <form method="POST" action="{{ route('kit-products.update', $kitProduct->id) }}" enctype="multipart/form-data"
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-10">
          ...
          <div class="space-y-8">
            ...
            <div>
              ...
              <input type="text" name="sku" value="{{ old('sku', $kitProduct->sku) }}" placeholder="e.g. KIT-SMPO-01" class="form-input" required>
            </div>
            <div>
              <label class="form-label font-semibold">Product Name *</label>
              <input type="text" name="name" value="{{ old('name', $kitProduct->name) }}" placeholder="e.g. Professional Shampoo" class="form-input" required>
            </div>
            <div class="grid grid-cols-2 gap-4">
              <div>
                <label class="form-label font-semibold">Brand</label>
                <input type="text" name="brand" value="{{ old('brand', $kitProduct->brand) }}" placeholder="e.g. L'Oreal" class="form-input">
              </div>
            <div>
              <label class="form-label font-semibold">Category *</label>
              <select name="category_id" class="form-input" required>
                @foreach($categories as $cat)
                  <option value="{{ $cat->id }}" {{ old('category_id', $kitProduct->category_id) == $cat->id ? 'selected' : '' }}>{{ $cat->name }}</option>
                @endforeach
              </select>
            </div>
            </div>
          ...
          <div class="space-y-8">
            ...
            <div class="grid grid-cols-2 gap-4">
              <div>
                ...
                <input type="number" step="0.01" name="price" value="{{ old('price', $kitProduct->price) }}" class="form-input">
              </div>
              <div>
                <label class="form-label font-semibold">Unit</label>
                <input type="text" name="unit" value="{{ old('unit', $kitProduct->unit) }}" placeholder="e.g. 500ml, pcs" class="form-input">
              </div>
            </div>
            <div class="grid grid-cols-2 gap-4">
              <div>
                <label class="form-label font-semibold">Total Stock *</label>
                <input type="number" name="total_stock" value="{{ old('total_stock', $kitProduct->total_stock) }}" class="form-input" required>
              </div>
              <div>
                <label class="form-label font-semibold">Min Stock Level</label>
                <input type="number" name="min_stock" value="{{ old('min_stock', $kitProduct->min_stock) }}" class="form-input">
              </div>
            </div>
            <div>
              <label class="form-label font-semibold">Expiry Date</label>
              <input type="date" name="expiry_date" value="{{ old('expiry_date', $kitProduct->expiry_date) }}" class="form-input">
            </div>
            ...
            <div class="flex items-center justify-between p-6 bg-[#F9F9F9] rounded-[1.5rem] border border-gray-50">
              ...
              <label class="toggle-switch">
                <input type="checkbox" name="status" value="Active" {{ old('status', $kitProduct->status) === 'Active' ? 'checked' : '' }}>
      ...
      <div class="flex items-center justify-end gap-3 px-10 py-6 bg-[#F9F9F9]/50">
        ...
        <button type="submit" class="btn btn-primary lg:px-10 shadow-lg shadow-black/10">
          <i data-lucide="check" class="w-4 h-4"></i> Update Product
        </button>
      </div>
    </form>
  </div>

...
@section('content')
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
      <div
        ...
        <div class="w-11 h-11 rounded-2xl bg-gray-100 flex items-center justify-center flex-shrink-0"><i
            data-lucide="shopping-bag" class="w-5 h-5 text-gray-600"></i></div>
      </div>
      <div
        class="stat-card bg-white rounded-2xl p-5 shadow-[0_2px_16px_rgba(0,0,0,0.04)] flex items-center justify-between gap-4">
        <div>
        ...
        <div class="w-11 h-11 rounded-2xl bg-emerald-50 flex items-center justify-center flex-shrink-0"><i
            data-lucide="indian-rupee" class="w-5 h-5 text-emerald-500"></i></div>
      </div>
      <div
        class="stat-card bg-white rounded-2xl p-5 shadow-[0_2px_16px_rgba(0,0,0,0.04)] flex items-center justify-between gap-4">
        <div>
          <p class="text-xs font-semibold text-gray-400 uppercase tracking-widest mb-1">Commission</p>
          <p class="text-3xl font-bold text-gray-900">₹{{ number_format($totalCommission) }}</p>
        </div>
        <div class="w-11 h-11 rounded-2xl bg-violet-50 flex items-center justify-center flex-shrink-0"><i
            data-lucide="percent" class="w-5 h-5 text-violet-500"></i></div>
      </div>
      <div
        class="stat-card bg-white rounded-2xl p-5 shadow-[0_2px_16px_rgba(0,0,0,0.04)] flex items-center justify-between gap-4">
        <div>
          <p class="text-xs font-semibold text-gray-400 uppercase tracking-widest mb-1">Pro Payouts</p>
          <p class="text-3xl font-bold text-gray-900">₹{{ number_format($totalPayouts) }}</p>
        </div>
        <div class="w-11 h-11 rounded-2xl bg-amber-50 flex items-center justify-center flex-shrink-0"><i
            data-lucide="wallet" class="w-5 h-5 text-amber-500"></i></div>
      </div>
    </div>
        ...
        <table class="w-full min-w-[1100px]">
          <thead>
            <tr class="border-b border-gray-100 bg-gray-50/80">
              ...
              <th class="px-5 py-4 text-left text-xs font-semibold text-gray-400 uppercase tracking-widest">Commission
              </th>
              <th class="px-5 py-4 text-left text-xs font-semibold text-gray-400 uppercase tracking-widest">Pro Earning
              </th>
              <th class="px-5 py-4 text-left text-xs font-semibold text-gray-400 uppercase tracking-widest">Order Status
              </th>
          ...
          <tbody id="ord-tbody">
            @foreach($orders as $ord)
              <tr class="table-row border-b border-gray-50" data-id="{{ $ord['id'] }}"
                data-search="{{ strtolower($ord['customer'] . ' ' . $ord['professional']) }}"
                data-customer="{{ $ord['customer'] }}" data-customer-phone="{{ $ord['customer_phone'] }}"
                data-address="{{ $ord['address'] }}" data-professional="{{ $ord['professional'] }}"
                data-service="{{ $ord['service'] }}" data-date="{{ $ord['date'] }}" data-time="{{ $ord['time'] }}"
                data-payment-method="{{ $ord['payment_method'] }}" data-amount="{{ $ord['amount'] }}"
                data-commission="{{ $ord['commission'] }}" data-pro-earning="{{ $ord['pro_earning'] }}"
                data-order-status="{{ $ord['order_status'] }}" data-payment-status="{{ $ord['payment_status'] }}">
  ...
  <!-- ── ORDER DETAIL DRAWER ─────────────────────────────────────────────── -->
  <div id="drawer-backdrop" class="fixed inset-0 z-[100] hidden bg-black/30 backdrop-blur-sm" onclick="closeDrawer()">
  </div>
  <div id="drawer-panel"
    class="drawer-panel fixed top-0 right-0 h-full w-full max-w-md bg-white z-[110] shadow-2xl flex flex-col overflow-hidden"
    style="z-index: 110;">
    <div class="flex items-center justify-between px-6 pt-6 pb-5 border-b border-gray-100 flex-shrink-0">
      ...
      <button onclick="closeDrawer()"
        class="w-9 h-9 rounded-xl bg-gray-100 hover:bg-gray-200 flex items-center justify-center transition-all">
    ...
    <div class="flex-1 overflow-y-auto p-6 flex flex-col gap-5">
      ...
      <div class="bg-black rounded-2xl p-5 grid grid-cols-3 gap-4">
        <div class="text-center">
          <p id="od-amount" class="text-xl font-bold text-white"></p>
          <p class="text-xs text-gray-400 mt-0.5">Total</p>
        </div>
        <div class="text-center border-x border-white/10">
          <p id="od-commission" class="text-xl font-bold text-violet-300"></p>
          <p class="text-xs text-gray-400 mt-0.5">Commission</p>
        </div>
        <div class="text-center">
          <p id="od-pro-earning" class="text-xl font-bold text-emerald-300"></p>
          <p class="text-xs text-gray-400 mt-0.5">Pro Payout</p>
        </div>
      </div>
      ...
      <div class="bg-gray-50 rounded-2xl p-4">
        ...
        <div class="flex items-start gap-2 mt-2 text-sm text-gray-500"><i data-lucide="map-pin"
            class="w-4 h-4 text-gray-400 flex-shrink-0 mt-0.5"></i><span id="od-address"></span></div>
      </div>
      ...
      <div class="bg-gray-50 rounded-2xl p-4">
        ...
        <div class="flex items-center gap-4 mt-2">
          <div class="flex items-center gap-1.5 text-sm text-gray-500"><i data-lucide="calendar"
              class="w-3.5 h-3.5 text-gray-400"></i><span id="od-date"></span></div>
          <div class="flex items-center gap-1.5 text-sm text-gray-500"><i data-lucide="clock"
              class="w-3.5 h-3.5 text-gray-400"></i><span id="od-time"></span></div>
        </div>
        <div class="flex items-center gap-1.5 text-sm text-gray-500 mt-1.5"><i data-lucide="credit-card"
            class="w-3.5 h-3.5 text-gray-400"></i><span id="od-payment-method"></span></div>
      </div>
      ...
      <div>
        ...
        <div class="grid grid-cols-3 gap-2">
          <button onclick="cancelOrder()"
            class="py-2.5 rounded-xl text-xs font-semibold border border-red-200 text-red-500 hover:bg-red-500 hover:text-white transition-all flex items-center justify-center gap-1.5">
            <i data-lucide="x-circle" class="w-3.5 h-3.5"></i> Cancel
          </button>
          <button onclick="refundOrder()"
            class="py-2.5 rounded-xl text-xs font-semibold border border-amber-200 text-amber-600 hover:bg-amber-500 hover:text-white transition-all flex items-center justify-center gap-1.5">
            <i data-lucide="rotate-ccw" class="w-3.5 h-3.5"></i> Refund
          </button>
          <button onclick="reassignOrder()"
            class="py-2.5 rounded-xl text-xs font-semibold border border-blue-200 text-blue-600 hover:bg-blue-500 hover:text-white transition-all flex items-center justify-center gap-1.5">
  ...
  <script>
    ...
    function openOrderDrawer(ord) {
      try {
        ...
        setText('od-id', ord.id);
        setText('od-customer', ord.customer);
        setText('od-customer-phone', ord.customerPhone);
        setText('od-address', ord.address);
        setText('od-professional', ord.professional);
        setText('od-service', ord.service);
        setText('od-time', ord.time);
        setText('od-payment-method', ord.paymentMethod);

        // Safe Date Parsing
        if (ord.date) {
          const d = new Date(ord.date);
          setText('od-date', !isNaN(d) ? d.toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' }) : ord.date);
        }

        // Safe Numeric Formatting
        const formatCurrency = (val) => '₹' + (parseFloat(val) || 0).toLocaleString('en-IN');
        setText('od-amount', formatCurrency(ord.amount));
        setText('od-commission', formatCurrency(ord.commission));
        setText('od-pro-earning', formatCurrency(ord.proEarning));
    ...
    function closeDrawer() { document.getElementById('drawer-panel').classList.remove('open'); document.getElementById('drawer-backdrop').classList.add('hidden'); document.body.style.overflow = ''; }

...
<div class="flex flex-col gap-6">
  ...
  <div class="flex items-center gap-4">
    ...
    <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold {{ $service->status === 'Active' ? 'bg-emerald-50 text-emerald-700 ring-1 ring-emerald-100' : 'bg-gray-100 text-gray-600' }}">{{ $service->status }}</span>
  </div>
  ...
  <form method="POST" action="{{ route('services.update', $service->id) }}" enctype="multipart/form-data" id="serviceForm">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div>
            ...
            <div class="relative mt-3 {{ $service->image ? '' : 'hidden' }}" id="serviceImgPreviewWrap">
              <img id="serviceImgPreview" class="w-full h-48 object-cover rounded-2xl border border-gray-100" src="{{ $service->image }}" alt="">
            ...
            <div class="mt-5">
              ...
              <textarea name="description" rows="4" class="form-input resize-none">{{ old('description', $service->description) }}</textarea>
            </div>
          </div>
        </div>
      </div>
            ...
            <div class="service-type-row dyn-item rounded-2xl border border-gray-200 p-5">
              <div class="flex items-center justify-between mb-4">
                <div class="flex items-center gap-2">
                  ...
                  <span class="text-xs font-semibold text-gray-500 uppercase tracking-wider svc-label">Service {{ $idx + 1 }}</span>
                </div>
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                ...
                <div>
                  ...
                  <input type="number" name="service_prices[]" value="{{ $type['price'] ?? '' }}" min="0" step="0.01" class="form-input text-right svc-price">
                </div>
              </div>
              <div class="mb-4">
                <label class="text-[11px] font-medium text-gray-500 uppercase tracking-wider mb-1 block">Reviews</label>
                <input type="number" name="service_reviews[]" value="{{ $type['reviews'] ?? '' }}" min="0" class="form-input">
              </div>
    ...
    <div class="bg-white rounded-[2rem] border border-gray-100 shadow-sm overflow-hidden mb-6 section-card">
      ...
      <div class="px-8 pb-8 space-y-5">
        <div>
          ...
          <input type="text" name="desc_title" value="{{ old('desc_title', $service->desc_title ?? '') }}" class="form-input">
        </div>
        <div>
    ...
    <div class="sticky-bar rounded-2xl border border-gray-100 shadow-lg px-8 py-4 flex items-center justify-between mt-2">
      <form action="{{ route('services.destroy', $service->id) }}" method="POST" class="inline" id="deleteForm">
        @csrf @method('DELETE')
        <button type="button" onclick="confirmDelete()" class="btn btn-danger text-sm"><i data-lucide="trash-2" class="w-4 h-4"></i> Delete</button>
      </form>
...
<style>
  ...
  .btn-danger { background:#fef2f2; color:#ef4444; border:1px solid #fee2e2; } .btn-danger:hover { background:#fee2e2; }
</style>
@endpush

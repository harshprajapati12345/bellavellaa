  ...
  <div class="flex flex-col gap-6">
    ...
    <div class="flex items-center gap-4">
      ...
      <div>
        ...
        <p class="text-sm text-gray-400 mt-0.5">{{ $offer->title }}</p>
      </div>
    </div>
    <form method="POST" action="{{ route('offers.update', $offer->id) }}" enctype="multipart/form-data">@csrf
      @method('PUT')
          ...
          <div class="space-y-5">
            ...
            <div class="grid grid-cols-2 gap-4">
              <div><label id="discount-value-label" class="block text-sm font-medium text-gray-700 mb-1">Discount
                  {{ $offer->discount_type === 'percentage' ? 'Percentage (%)' : 'Amount (₹)' }}</label><input
                  type="number" name="discount_value" value="{{ old('discount_value', $offer->discount_value) }}" min="0"
                  class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:border-black outline-none transition-all">
              </div>
            ...
            <div><label class="block text-sm font-medium text-gray-700 mb-1">Promo Code</label><input type="text"
                name="code" value="{{ old('code', $offer->code) }}"
            ...
            <div class="grid grid-cols-2 gap-4">
              <div><label class="block text-sm font-medium text-gray-700 mb-1">Valid From</label><input type="date"
                  name="valid_from" value="{{ old('valid_from', $offer->valid_from) }}"
                  class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:border-black outline-none transition-all">
              </div>
              <div><label class="block text-sm font-medium text-gray-700 mb-1">Valid Until</label><input type="date"
                  name="valid_until" value="{{ old('valid_until', $offer->valid_until) }}"
                  class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:border-black outline-none transition-all">
              </div>
            </div>
          </div>
          <div class="space-y-5">
            <div><label class="block text-sm font-medium text-gray-700 mb-1">Image</label>@if($offer->image)<img
              src="{{ asset('storage/' . $offer->image) }}"
            class="w-full h-32 object-cover rounded-xl border border-gray-100 mb-2" alt="">@endif<input type="file"
            ...
            <label class="flex items-center gap-2 cursor-pointer"><input type="checkbox" name="status" {{ $offer->status === 'Active' ? 'checked' : '' }} class="w-4 h-4 accent-black"><span
      ...
      <div class="flex items-center justify-between">
        <form action="{{ route('offers.destroy', $offer->id) }}" method="POST" class="inline"
          onsubmit="return confirm('Delete?')">@csrf @method('DELETE')<button type="submit"
            class="px-4 py-2.5 rounded-xl bg-red-50 text-red-600 border border-red-100 hover:bg-red-100 transition-all text-sm font-medium flex items-center gap-2"><i
              data-lucide="trash-2" class="w-4 h-4"></i> Delete</button></form>
        <div class="flex gap-3"><a href="{{ route('offers.index') }}"
            ...
            type="submit"
            class="px-6 py-3 rounded-xl bg-black text-white font-medium hover:bg-gray-800 transition-all">Update
            Offer</button></div>
      </div>
    </form>
  </div>

    ...
    <div class="flex flex-col gap-6">
      ...
      <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div class="stat-card bg-white rounded-2xl p-5 shadow-[0_2px_16px_rgba(0,0,0,0.04)] flex items-center justify-between gap-4">
          <div><p class="text-xs font-semibold text-gray-400 uppercase tracking-widest mb-1">Total</p><p class="text-3xl font-bold text-gray-900">{{ $total }}</p><p class="text-xs text-gray-400 mt-0.5">Bookings</p></div>
        ...
        <div class="stat-card bg-white rounded-2xl p-5 shadow-[0_2px_16px_rgba(0,0,0,0.04)] flex items-center justify-between gap-4">
          <div><p class="text-xs font-semibold text-amber-500 uppercase tracking-widest mb-1">Unassigned</p><p class="text-3xl font-bold text-gray-900">{{ $unassigned }}</p><p class="text-xs text-gray-400 mt-0.5">Need action</p></div>
        ...
        <div class="stat-card bg-white rounded-2xl p-5 shadow-[0_2px_16px_rgba(0,0,0,0.04)] flex items-center justify-between gap-4">
          <div><p class="text-xs font-semibold text-violet-500 uppercase tracking-widest mb-1">In Progress</p><p class="text-3xl font-bold text-gray-900">{{ $inProgress }}</p><p class="text-xs text-gray-400 mt-0.5">Active now</p></div>
        ...
        <div class="stat-card bg-white rounded-2xl p-5 shadow-[0_2px_16px_rgba(0,0,0,0.04)] flex items-center justify-between gap-4">
          <div><p class="text-xs font-semibold text-emerald-500 uppercase tracking-widest mb-1">Completed</p><p class="text-3xl font-bold text-gray-900">{{ $completed }}</p><p class="text-xs text-gray-400 mt-0.5">Done</p></div>
      ...
      <div class="bg-white rounded-2xl shadow-[0_2px_16px_rgba(0,0,0,0.04)] overflow-hidden">
          ...
          <table class="w-full min-w-[900px]">
            ...
            <tbody id="assign-tbody">
              ...
              <tr class="table-row border-b border-gray-50"
                  data-id="{{ $b->id }}"
                  data-customer="{{ strtolower($b->customer ?? $b->customer_name ?? '') }}"
                  data-display-name="{{ $b->customer ?? $b->customer_name }}"
                  data-service="{{ strtolower($b->service ?? $b->service_name ?? '') }}"
                  data-display-service="{{ $b->service ?? $b->service_name }}"
                  data-package="{{ $b->package ?? '' }}"
                  data-status="{{ $b->status }}"
                  data-avatar="{{ $b->avatar ?? $b->customer_avatar ?? 'https://i.pravatar.cc/64?img='.($b->id % 50) }}"
                  data-city="{{ $b->city ?? '—' }}"
                  data-date="{{ $b->date ? \Carbon\Carbon::parse($b->date)->format('d M Y') : '—' }}"
                  data-slot="{{ $b->slot ?? '' }}"
                  data-professional="{{ $b->professional ?? '' }}">
                <td class="px-5 py-4"><span class="text-xs font-semibold text-gray-400">#{{ $b->id }}</span></td>
                <td class="px-5 py-4">
                  <div class="flex items-center gap-3">
                    <img src="{{ $b->avatar ?? $b->customer_avatar ?? 'https://i.pravatar.cc/64?img='.($b->id % 50) }}" class="w-9 h-9 rounded-full object-cover flex-shrink-0 ring-2 ring-white shadow-sm" alt="">
                    <div>
                      <p class="text-sm font-semibold text-gray-900">{{ $b->customer ?? $b->customer_name }}</p>
                      <p class="text-xs text-gray-400">{{ $b->city ?? '—' }}</p>
                    </div>
                  </div>
                </td>
                <td class="px-5 py-4">
                  <p class="text-sm font-medium text-gray-900">{{ $b->service ?? $b->service_name }}</p>
                  <p class="text-xs text-gray-400 mt-0.5">{{ $b->package ?? '' }}</p>
                </td>
                <td class="px-5 py-4">
                  <p class="text-sm font-medium text-gray-900">{{ $b->date ? \Carbon\Carbon::parse($b->date)->format('d M Y') : '—' }}</p>
                  <p class="text-xs text-gray-400 mt-0.5">{{ $b->slot ?? '' }}</p>
                </td>
                <td class="px-5 py-4">
                  @if($b->professional)
                  <p class="text-sm font-medium text-gray-900">{{ $b->professional }}</p>
                  @else
                  <span class="text-xs text-gray-400 italic">Not assigned</span>
                  @endif
                </td>
                <td class="px-5 py-4">
                  <span class="inline-flex items-center gap-1.5 text-xs font-semibold px-2.5 py-1 rounded-full {{ $statusColors[$b->status] ?? 'bg-gray-100 text-gray-500' }}">
                    <span class="w-1.5 h-1.5 rounded-full {{ $statusDots[$b->status] ?? 'bg-gray-300' }}"></span>{{ $b->status }}
                  </span>
                </td>
                <td class="px-5 py-4">
                  <div class="flex items-center justify-end gap-1.5">
                    ...
                    <button type="button" onclick="openAssignDrawer(this.closest('tr').dataset)"
                      ...
                      class="flex items-center gap-1.5 px-3 py-1.5 rounded-lg {{ $b->status === 'Unassigned' ? 'bg-black text-white hover:bg-gray-800' : 'border border-gray-200 text-gray-600 hover:bg-gray-100' }} text-xs font-medium transition-all">
                      <i data-lucide="user-plus" class="w-3.5 h-3.5"></i>
                      {{ $b->status === 'Unassigned' ? 'Assign' : 'Reassign' }}
                    </button>
                    @elseif($b->status === 'In Progress')
                    <span class="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-violet-50 text-violet-600 text-xs font-medium"><i data-lucide="loader" class="w-3.5 h-3.5"></i> In Progress</span>
                    @else
                    <span class="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-emerald-50 text-emerald-600 text-xs font-medium"><i data-lucide="check" class="w-3.5 h-3.5"></i> Done</span>
                    @endif
                  </div>
                </td>
              </tr>
        ...
        <div id="empty-state" class="hidden flex-col items-center justify-center py-20 text-center">
          <div class="w-16 h-16 bg-gray-100 rounded-2xl flex items-center justify-center mb-4 mx-auto"><i data-lucide="clipboard-x" class="w-8 h-8 text-gray-300"></i></div>
...
<div id="drawer-panel" class="drawer-panel closed fixed top-0 right-0 h-full w-full max-w-md bg-white z-50 shadow-2xl flex flex-col overflow-hidden">
  ...
  <div class="px-6 pt-6 pb-5 border-b border-gray-100 flex items-center justify-between flex-shrink-0">
    ...
    <button onclick="closeAssignDrawer()" class="w-9 h-9 rounded-xl bg-gray-50 hover:bg-gray-100 flex items-center justify-center transition-all">
  ...
  <div class="flex-1 overflow-y-auto p-6 space-y-3" id="pro-list">
    ...
    <div class="pro-card p-4 rounded-2xl border-2 border-transparent bg-white shadow-sm ring-1 ring-gray-100 flex items-center gap-4 hover:shadow-md transition-all"
...
<style>
  ...
  .filter-tab { transition: all 0.2s; border-radius: 0.75rem; } .filter-tab.active { background: #000; color: #fff; }
  .filter-tab:not(.active) { color: #6b7280; } .filter-tab:not(.active):hover { background: #f3f4f6; color: #111; }

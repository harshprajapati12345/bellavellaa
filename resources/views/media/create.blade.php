  ...
  <div class="flex flex-col gap-6">
    ...
    <form method="POST" action="{{ route('media.store') }}" enctype="multipart/form-data" id="mediaForm">
      ...
      <div class="flex flex-col gap-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
              ...
              <div>
                ...
                <select name="media_type" id="media-type" required
                  ...
                  <option value="banner" {{ old('media_type', request('type')) === 'banner' ? 'selected' : '' }}>Image
                    (Banner)</option>
                  <option value="video" {{ old('media_type', request('type')) === 'video' ? 'selected' : '' }}>Video
                  </option>
                </select>
              </div>
              ...
              <div>
                <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2 ml-1">Linked Section
                  <span class="text-red-400">*</span></label>
                <select name="linked_section" required
                  class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-black/5 focus:border-black/50 text-sm bg-white cursor-pointer transition-all">
                  <option value="">Select section...</option>
                  @foreach(['Top Banner', 'Mid Section', 'Side Gallery', 'Bottom Footer', 'Hero Slider', 'Services'] as $s)
                    <option value="{{ $s }}" {{ (old('linked_section', request('section')) === $s) ? 'selected' : '' }}>
                      {{ $s }}</option>
                  @endforeach
                </select>
              </div>
              ...
              <div>
                ...
                <select name="target_page" required
                  ...
                  @foreach(['Home', 'Services', 'Packages', 'About Us', 'Contact', 'Professionals', 'Offers'] as $pg)
                    <option value="{{ $pg }}" {{ (old('target_page', request('page')) === $pg) ? 'selected' : '' }}>{{ $pg }}
                    </option>
                  @endforeach
                </select>
              </div>
          ...
          <div class="px-8 pb-8">
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 items-start">
              ...
              <div>
                ...
                <!-- Type hint: image -->
                <div id="hint-image"
                  class="{{ request('type') === 'banner' ? '' : 'hidden' }} mb-3 flex items-center gap-2 px-4 py-2.5 bg-blue-50 rounded-xl">
                ...
                <!-- Type hint: video -->
                <div id="hint-video"
                  class="{{ request('type') === 'video' ? '' : 'hidden' }} mb-3 flex items-center gap-2 px-4 py-2.5 bg-violet-50 rounded-xl">
                ...
                <div id="mainDropZone"
                  ...
                  <input type="file" id="file-input" name="media_file" class="hidden" accept="image/*"
                    onchange="previewUpload(this)">
                </div>

                <!-- Image preview -->
                <div class="relative mt-3 hidden" id="imgPreviewWrap">
                  <img id="preview-img" class="w-full max-h-52 object-cover rounded-2xl border border-gray-100" src=""
                    alt="">
              ...
              <div class="space-y-5">
                ...
                <div id="thumbnail-wrap" class="{{ request('type') === 'video' ? '' : 'hidden' }}">
                  <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2 ml-1">Video
                    Thumbnail <span class="text-xs text-gray-400 font-normal">(Optional)</span></label>
                  <div id="thumbDropZone"
                    ...
                    ondragover="event.preventDefault(); this.classList.add('border-black', 'bg-white')"
                    ondragleave="this.classList.remove('border-black', 'bg-white')" ondrop="handleThumbDrop(event)">
                    ...
                    <input type="file" id="thumb-input" name="thumbnail" class="hidden" accept="image/*"
        ...
        <!-- STICKY ACTION BAR -->
        <div
          class="sticky bottom-8 z-30 bg-white/80 backdrop-blur-xl rounded-2xl border border-gray-100 shadow-xl px-8 py-4 flex items-center justify-end gap-3">
          <a href="{{ route('media.index') }}"

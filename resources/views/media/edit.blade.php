@extends('layouts.app')
@php $pageTitle = 'Edit Media'; @endphp
...
@section('content')
  <div class="flex flex-col gap-6">
    ...
    <div class="flex items-center gap-4">
      ...
      <div>
        <h2 class="text-2xl font-semibold text-gray-900 tracking-tight">Edit Media</h2>
        <p class="text-sm text-gray-400 mt-0.5">{{ $media->title }}</p>
      </div>
    </div>
    ...
    <form method="POST" action="{{ route('media.update', $media->id) }}" enctype="multipart/form-data" id="mediaForm">
      @csrf
      @method('PUT')
      <div class="flex flex-col gap-6">
          ...
          <div class="px-8 pb-8">
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
              ...
              <div>
                ...
                <select name="homepage_content_id" id="linked-section" required
                  ...
                  @foreach($sections as $s)
                    <option value="{{ $s->id }}" data-media-type="{{ $s->content['media_type'] ?? 'image' }}" {{ (old('homepage_content_id', $media->homepage_content_id) == $s->id) ? 'selected' : '' }}>
              ...
              <div>
                ...
                <input type="hidden" name="media_type" id="media-type-value" value="{{ old('media_type', $media->type) }}">
              </div>
              ...
              <div>
                ...
                <input type="number" name="order" min="0"
                  class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-black/5 focus:border-black/50 text-sm bg-white transition-all"
                  placeholder="e.g. 1" value="{{ old('order', $media->order) }}">
              </div>
              ...
              <div class="flex items-start pt-1">
                <div class="w-full py-3 px-4 bg-gray-50 rounded-xl flex items-center justify-between">
                  ...
                  <label class="relative inline-flex items-center cursor-pointer">
                    <input type="checkbox" name="status" class="sr-only peer" {{ old('status', $media->status === 'Active') ? 'checked' : '' }}>
                    <div
        ...
        <div class="bg-white rounded-[2rem] border border-gray-100 shadow-sm overflow-hidden section-card max-w-4xl mx-auto w-full">
          <div class="px-8 pt-7 pb-2">
            <div class="flex items-center gap-3 mb-6">
              ...
              <h3 class="text-sm font-semibold text-gray-900 uppercase tracking-widest">Update File</h3>
              <div class="flex-1 h-px bg-gray-100 ml-2"></div>
            </div>
          </div>
          <div class="px-8 pb-8">
              ...
              <div>
                ...
                <div id="mainDropZone"
                  ...
                  <div id="main-placeholder" class="flex flex-col items-center gap-3 py-8 px-4 text-center">
                    ...
                    <div>
                      <p class="text-sm font-semibold text-gray-700">Click to upload or drag &amp; drop to replace current file</p>
                      <p id="upload-sub-text" class="text-xs text-gray-400 mt-1">JPG, PNG, GIF, MP4, WebM — Max 50MB</p>
                    </div>
                  </div>

                  <!-- Image/Current Media Preview (Hidden by default, shown inside box when selected) -->
                  ...
                  <div class="relative hidden w-full h-full py-8 text-center" id="videoPreviewWrap">
                    <div class="flex flex-col items-center gap-3 px-4">
                      ...
                      <div>
                        ...
                        <p class="text-xs text-gray-400 mt-1" id="video-filesize"></p>
                      </div>
                    </div>
                    <button type="button" onclick="removeMainFile(event)"
                      class="absolute top-4 right-4 w-8 h-8 rounded-full bg-white/90 border border-gray-200 flex items-center justify-center text-gray-500 hover:bg-red-50 hover:text-red-500 hover:border-red-200 transition-all shadow-sm">
        ...
        <div
          ...
          <button type="button" onclick="document.getElementById('delete-media-form').submit();"
            class="px-6 py-2.5 rounded-xl border border-red-200 text-red-600 font-medium hover:bg-red-50 transition-all mr-auto flex items-center gap-2">
          ...
          <button type="submit"
            ...
            <i data-lucide="save" class="w-4 h-4 text-white"></i> Update Media
          </button>
        </div>
    ...
    <form action="{{ route('media.destroy', $media->id) }}" method="POST" id="delete-media-form" class="hidden"
        onsubmit="return confirm('Really delete and remove this media permanently?')">
        @csrf
        @method('DELETE')
    </form>
...
@push('scripts')
  <script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Show existing media if it exists
        if (existingFileUrl) {
            ...
            if (existingFileType.toLowerCase() === 'banner' || existingFileType.toLowerCase() === 'image') {
                document.getElementById('preview-img').src = existingFileUrl;
                ...
                document.getElementById('video-filename').innerHTML = '<a href="'+existingFileUrl+'" target="_blank" class="text-blue-600 hover:underline">View Current Video</a>';
                document.getElementById('video-filesize').textContent = 'Current File Linked';

    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
      <div
        ...
        <div>
          ...
          <p class="text-xs text-gray-400 mt-0.5">Packages</p>
        </div>
        <div class="w-11 h-11 rounded-2xl bg-gray-100 flex items-center justify-center flex-shrink-0"><i
            data-lucide="package" class="w-5 h-5 text-gray-600"></i></div>
      </div>
      <div
        class="stat-card bg-white rounded-2xl p-5 shadow-[0_2px_16px_rgba(0,0,0,0.04)] flex items-center justify-between gap-4">
        <div>
      ...
      <div
        ...
        <div>
          ...
          <p class="text-3xl font-bold text-gray-900">{{ $inactive }}</p>
          <p class="text-xs text-gray-400 mt-0.5">Paused</p>
        </div>
        <div class="w-11 h-11 rounded-2xl bg-gray-100 flex items-center justify-center flex-shrink-0"><i
            data-lucide="pause-circle" class="w-5 h-5 text-gray-400"></i></div>
      </div>
      <div
        class="stat-card bg-white rounded-2xl p-5 shadow-[0_2px_16px_rgba(0,0,0,0.04)] flex items-center justify-between gap-4">
        <div>
          <p class="text-xs font-semibold text-violet-500 uppercase tracking-widest mb-1">Top Booked</p>
          @if($topBooked)
            <p class="text-lg font-bold text-gray-900 leading-tight mt-0.5">{{ $topBooked->name }}</p>
          <p class="text-xs text-gray-400 mt-0.5">{{ $topBooked->bookings }} bookings</p>@else<p
          class="text-lg font-bold text-gray-900">–</p>@endif
        </div>
        <div class="w-11 h-11 rounded-2xl bg-violet-50 flex items-center justify-center flex-shrink-0"><i
            data-lucide="trending-up" class="w-5 h-5 text-violet-500"></i></div>
      </div>
    </div>

    <!-- Filter Bar -->
    <div id="filter-bar"
      class="filter-bar bg-white rounded-2xl p-4 shadow-[0_2px_16px_rgba(0,0,0,0.04)] flex flex-wrap items-center gap-3">
      ...
      <select id="f-featured" onchange="applyFilters()"
        class="px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm text-gray-700 focus:outline-none focus:ring-2 focus:ring-black/5 cursor-pointer">
        <option value="">All</option>
        <option value="1">Featured Only</option>
      </select>
      <button onclick="resetFilters()"
        class="flex items-center gap-2 px-4 py-2.5 rounded-xl border border-gray-200 text-sm text-gray-500 hover:bg-gray-100 transition-all ml-auto">
    ...
    <div class="bg-white rounded-2xl shadow-[0_2px_16px_rgba(0,0,0,0.04)] overflow-hidden">
      ...
      <div id="bulk-bar" class="bulk-bar hidden items-center gap-3 px-5 py-3 bg-gray-900 text-white">
        ...
        <button onclick="bulkDelete()"
          class="ml-auto flex items-center gap-2 px-4 py-2 rounded-xl bg-red-500 hover:bg-red-600 text-white text-sm font-medium transition-all">
          <i data-lucide="trash-2" class="w-4 h-4"></i> Delete Selected
        </button>
        <button onclick="clearSelection()"
          class="flex items-center gap-2 px-4 py-2 rounded-xl bg-white/10 hover:bg-white/20 text-sm font-medium transition-all">
        ...
        <table class="w-full min-w-[1000px]">
          <thead>
            <tr class="border-b border-gray-100 bg-gray-50/80">
              <th class="px-5 py-4 text-left w-10"><input type="checkbox" id="select-all" onchange="toggleAll(this)"
                  class="w-4 h-4 rounded border-gray-300 cursor-pointer accent-black"></th>
            ...
            @foreach($packages as $pkg)
              ...
              <tr class="table-row border-b border-gray-50" data-id="{{ $pkg->id }}"
                ...
                <td class="px-5 py-4">
                  <div class="flex items-center gap-3">
                    <img
                      src="{{ $pkg->image ?: 'https://images.unsplash.com/photo-1596704017254-9b1b1b9e07f9?auto=format&fit=crop&w=400&q=80' }}"
                      class="w-10 h-10 rounded-xl object-cover flex-shrink-0" alt="">
                    <div>
                      <p class="text-sm font-semibold text-gray-900">{{ $pkg->name }}</p>
                      @if($pkg->featured)<span
                      class="text-xs font-medium text-amber-600 bg-amber-50 px-2 py-0.5 rounded-full">Featured</span>@endif
                    </div>
                  </div>
                </td>
                <td class="px-5 py-4">
                ...
                <td class="px-5 py-4">
                  <label class="toggle-switch">
                    <input type="checkbox" {{ $pkg->status === 'Active' ? 'checked' : '' }}
                      onchange="toggleStatus({{ $pkg->id }}, this)">
                    <span class="toggle-slider"></span>
                  </label>
                </td>
                <td class="px-5 py-4 text-sm text-gray-400">{{ \Carbon\Carbon::parse($pkg->created_at)->format('d M Y') }}
                </td>
                <td class="px-5 py-4">
                  <div class="flex items-center justify-end gap-1.5">
                    ...
                    <button type="button" onclick="deletePackage({{ $pkg->id }}, '{{ addslashes($pkg->name) }}')"
                      title="Delete"
      ...
      <div id="empty-state" class="hidden flex-col items-center justify-center py-20 text-center">
        ...
        <button onclick="resetFilters()"
          class="mt-4 px-5 py-2.5 rounded-xl bg-black text-white text-sm font-medium hover:bg-gray-800 transition-all">Reset
          Filters</button>
      </div>

    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
      <div
        ...
        <div>
          ...
          <p class="text-xs text-gray-400 mt-0.5">Active & Inactive</p>
        </div>
        <div class="w-11 h-11 rounded-2xl bg-gray-100 flex items-center justify-center flex-shrink-0"><i data-lucide="tag"
            class="w-5 h-5 text-gray-600"></i></div>
      </div>
      <div
        class="stat-card bg-white rounded-2xl p-5 shadow-[0_2px_16px_rgba(0,0,0,0.04)] flex items-center justify-between gap-4">
        <div>
          <p class="text-xs font-semibold text-emerald-500 uppercase tracking-widest mb-1">Live Now</p>
          <p class="text-3xl font-bold text-gray-900">{{ $active }}</p>
          <p class="text-xs text-gray-400 mt-0.5">Current Offers</p>
        </div>
      ...
      <div
        class="stat-card bg-white rounded-2xl p-5 shadow-[0_2px_16px_rgba(0,0,0,0.04)] flex items-center justify-between gap-4">
        <div>
          <p class="text-xs font-semibold text-blue-500 uppercase tracking-widest mb-1">Total Usage</p>
          <p class="text-3xl font-bold text-gray-900">{{ number_format($totalUsage) }}</p>
          <p class="text-xs text-gray-400 mt-0.5">All time claims</p>
        </div>
        <div class="w-11 h-11 rounded-2xl bg-blue-50 flex items-center justify-center flex-shrink-0"><i
            data-lucide="ticket" class="w-5 h-5 text-blue-500"></i></div>
      </div>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-[0_2px_16px_rgba(0,0,0,0.04)] overflow-hidden">
      <div class="overflow-x-auto">
        <table class="w-full text-left">
          ...
          <tbody>
            @foreach($offers as $o)
              <tr class="table-row border-b border-gray-50" data-id="{{ $o->id }}"
                data-name="{{ strtolower($o->name ?? '') }}" data-display-name="{{ $o->name }}"
                ...
                <td class="px-4 py-4"><span class="text-sm font-semibold text-gray-900">{{ $o->name }}</span></td>
                <td class="px-4 py-4">
                  <span
                    class="text-xs font-mono bg-gray-100 px-3 py-1 rounded-lg text-gray-600 font-bold border border-gray-200">{{ $o->code ?? '—' }}</span>
                </td>
                <td class="px-4 py-4">
                  <div class="flex items-center gap-2">
                    <span
                      class="text-sm font-bold text-gray-900">{{ $o->discount_type === 'percentage' ? $o->discount_value . '%' : '₹' . $o->discount_value }}</span>
                    <span
                      class="text-[10px] text-gray-400 bg-gray-50 px-1.5 py-0.5 rounded border border-gray-100">{{ ucwords($o->discount_type ?? 'Percentage') }}</span>
                  </div>
                </td>
                ...
                <td class="px-4 py-4">
                  <label class="toggle-switch">
                    <input type="checkbox" {{ ($o->status ?? 'Active') === 'Active' ? 'checked' : '' }}
                      onchange="toggleStatus({{ $o->id }}, this)">
                ...
                <td class="px-4 py-4 text-right">
                  <div class="flex items-center justify-end gap-1.5">
                    <button
                      class="view-btn w-8 h-8 flex items-center justify-center rounded-lg text-gray-400 hover:text-black hover:bg-gray-100 transition-colors"
                      data-id="{{ $o->id }}" data-type="offers" title="View Details">
                    ...
                    <button onclick="deleteOffer({{ $o->id }}, '{{ addslashes($o->name ?? '') }}')" title="Delete"
